...
tmux send-keys -t "$SESSION_NAME:0.1" "source $ROOT_DIR/servers/VerifyServer/.venv/bin/activate" C-m
tmux send-keys -t "$SESSION_NAME:0.1" "cd $ROOT_DIR/servers/VerifyServer" C-m
tmux send-keys -t "$SESSION_NAME:0.1" "python3 verify_server.py" C-m

# 窗格 2: 启动 GateServer
tmux split-window -v -t "$SESSION_NAME:0.1"
echo "Starting GateServer..."
tmux send-keys -t "$SESSION_NAME:0.2" "cd $ROOT_DIR/build/servers/GateServer" C-m
tmux send-keys -t "$SESSION_NAME:0.2" "./GateServer" C-m
